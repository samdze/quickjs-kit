...
struct TypeScriptToolingExamplesTests {
    private struct UsersUser: Codable, Sendable, TypeScriptSchemaProviding {
        let id: Int
    ...
    private struct BillingUser: Codable, Sendable, TypeScriptSchemaProviding {
        let id: Int

        static let typeScriptSchema = TypeScriptSchema.interface(
            "User",
            scope: .module("host:billing"),
            properties: [.init("id", type: .number)]
        )
    }
    ...
    private struct GlobalConfiguration: Codable, Sendable, TypeScriptSchemaProviding {
        let environment: String

        static let typeScriptSchema = TypeScriptSchema.interface(
            "GlobalConfiguration",
            scope: .global,
            properties: [.init("environment", type: .string)]
        )
    }

    private struct Order: Codable, Sendable, TypeScriptSchemaProviding {
        let user: UsersUser
        ...
        static let typeScriptSchema = TypeScriptSchema.interface(
            "Order",
            scope: .module("host:orders"),
            properties: [
                .init(
                    "user",
                    type: .named("User", scope: .module("host:users"))
                ),
                .init(
                    "billingUser",
                    type: .named("User", scope: .module("host:billing")),
                    isOptional: true
                ),
                .init(
                    "configuration",
                    type: .named("GlobalConfiguration", scope: .global)
                ),
                .init(
                    "users",
                    type: .array(.named("User", scope: .module("host:users")))
                ),
                .init(
                    "byID",
                    type: .record(.named("User", scope: .module("host:users")))
                ),
                .init(
                    "state",
    ...
    @Test("cross-scope declarations render as one stable complete file")
    func crossScopeDeclarationsRenderAsStableGolden() async throws {
        let runtime = try JavaScriptRuntime()
        try await runtime.function("configuration") {
            GlobalConfiguration(environment: "test")
        }
        try await runtime.function("cachedUser") {
            UsersUser(id: 0)
        }
        try await runtime.defineModule("host:orders") { module in
            module.function("load") {
                Order(
                    user: UsersUser(id: 1),
                    billingUser: BillingUser(id: 2),
                    configuration: GlobalConfiguration(environment: "test")
                )
            }
        }
        try await runtime.defineModule("host:users") { module in
            module.function("load") { UsersUser(id: 1) }
        }
        try await runtime.defineModule("host:billing") { module in
            module.function("load") { BillingUser(id: 2) }
        }
        ...
        declare function cachedUser(): import("host:users").User;

        declare function configuration(): GlobalConfiguration;
        ...
        declare module "host:orders" {
            ...
            export interface Order {
                ...
                users: __qjs_User_from_host_users[];
                byID: Record<string, __qjs_User_from_host_users>;
                state: string | null;
            }
    ...
    @Test("module imports are grouped and aliases are deterministic")
    func moduleImportsAreGroupedAndAliased() throws {
        let template = try JavaScriptRuntimeTemplate {
            SwiftModule("host:users") {
                Function("load") { UsersUser(id: 1) }
            }
            SwiftModule("host:billing") {
                Function("load") { BillingUser(id: 2) }
            }
            SwiftModule("host:orders") {
                Function("user") { UsersUser(id: 3) }
                Function("billingUser") { BillingUser(id: 4) }
            }
        }
        ...
        #expect(
            declarations.contains(
                "export function user(): __qjs_User_from_host_users;"
            )
        )
        #expect(
            declarations.contains(
                "export function billingUser(): __qjs_User_from_host_billing;"
            )
        )
    }

    @Test("imports are aliased when a module member shadows the imported name")
    func moduleImportsAvoidLocalNameCollisions() throws {
        let template = try JavaScriptRuntimeTemplate {
            Globals {
                Function("User") { 1 }
            }
            ...
            SwiftModule("host:orders") {
                Function("User") { UsersUser(id: 2) }
            }
        }

        let declarations = try template.environmentDescription().typeScriptDeclarations()
        #expect(declarations == """
            // Generated by QuickJSKit. Do not edit.

            declare function User(): number | bigint;
    ...
    @Test("cross-scope declaration output is independent of registration order")
    func crossScopeDeclarationOrderIsStable() async throws {
        let first = try JavaScriptRuntime()
        try await first.function("configuration") {
            GlobalConfiguration(environment: "test")
        }
        try await first.defineModule("host:users") { module in
            module.function("load") { UsersUser(id: 1) }
        }
        try await first.defineModule("host:billing") { module in
            module.function("load") { BillingUser(id: 1) }
        }
        try await first.defineModule("host:orders") { module in
            module.function("load") {
                Order(
                    user: UsersUser(id: 1),
                    billingUser: nil,
        ...
        try await second.defineModule("host:orders") { module in
            module.function("load") {
                Order(
                    user: UsersUser(id: 1),
                    billingUser: nil,
        ...
        try await second.defineModule("host:users") { module in
            module.function("load") { UsersUser(id: 1) }
        }
        try await second.defineModule("host:billing") { module in
            module.function("load") { BillingUser(id: 1) }
        }
        try await second.function("configuration") {
            GlobalConfiguration(environment: "test")
        }

        let firstDeclarations = try await first.environmentDescription()
            .typeScriptDeclarations()
        let secondDeclarations = try await second.environmentDescription()
            .typeScriptDeclarations()
        #expect(firstDeclarations == secondDeclarations)
    }
    ...
    func snapshotRendersTypedGlobals() async throws {
         ...
         */
        declare function loadUser(id: number | bigint, nickname?: string | null): Promise<User | null>;
        """ + "\n")
    }
    ...
    func structuredTSDocDescribesFunctions() async throws {
         ...
         * @deprecated Use {@link fetchUser} instead.
         */
        declare function loadUser(id: number | bigint): Promise<User | null>;
        """))
    }
    ...
    func exportsShareSchemaMetadata() async throws {
        ...
        #expect(declarations.contains("declare let currentUser: User;"))
        #expect(declarations.contains("declare const directory: {"))
        #expect(declarations.contains("user: (id: number | bigint) => Promise<User>;"))
        #expect(declarations.contains("readonly version: string;"))
        #expect(declarations.contains("declare module \"host:users\""))
        #expect(declarations.contains("export const featured: User;"))
    }
