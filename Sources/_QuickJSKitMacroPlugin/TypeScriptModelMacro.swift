...
public struct TypeScriptModelMacro: ExtensionMacro {
    public static func expansion(
        ...
        if let classDeclaration = declaration.as(ClassDeclSyntax.self),
           !classDeclaration.modifiers.contains(where: { $0.name.text == "final" }) {
            throw QuickJSKitMacroError("@TypeScriptModel requires classes to be final.")
        }
        guard declaration.is(StructDeclSyntax.self) || declaration.is(ClassDeclSyntax.self) else {
            throw QuickJSKitMacroError(
                "@TypeScriptModel supports Codable structs, final classes, and raw String or Int enums."
            )
        }
    ...
    private static func enumExtension(
        ...
        guard isString || isInteger else {
            throw QuickJSKitMacroError(
                "@TypeScriptModel enums require a raw String or Int representation."
            )
        }
        ...
        for member in declaration.memberBlock.members {
            ...
            for element in caseDeclaration.elements {
                ...
                } else if let literal = element.rawValue?.value.as(IntegerLiteralExprSyntax.self),
                          let integer = Int64(literal.literal.text) {
                    nextInteger = integer
                    value = ".integer(\(integer))"
                    nextInteger += 1
                } else if element.rawValue == nil {
                    value = ".integer(\(nextInteger))"
                    nextInteger += 1
                } else {
                    throw QuickJSKitMacroError(
                        "Raw Int enum values used by @TypeScriptModel must be integer literals."
                    )
                }
